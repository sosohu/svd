...
/*
void orth_row(int i, int j,int M,int N, double* A,double* P,double* Q,int MlsN,int* cond, double* tol_orth,int iteration,double last_ave_orth)
{
  int k,w;
			  ...
			  int* cond,double* tol_orth,int iteration,double last_ave_orth)
{
  ...
  for(i=start; i<end; i++)
   for(j=i+1; j<=end; j++)	 
   {
     orth_row(i,j,M,N,A,P,Q,MlsN,cond,tol_orth,iteration,last_ave_orth);
   }//end for for
}
